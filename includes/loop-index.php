...
if (have_posts()) :
	...
	if( strpos($blog_global_style, 'elegant-blog') !== false )
        ...
        echo "<div class='entry-content-wrapper clearfix {$post_format}-content'>";
           ...
           if(get_post_meta($the_id,'_listing_image_id') ){
			$post_details = get_post_meta($the_id,'_listing_image_id');
          	$image_url = wp_get_attachment_image_src($post_details[0],array(200,200));
			if(!empty($image_url)){
			/* set custom meta box image */
            ...
            $content_output  =  '<div class="vv entry-content" '.avia_markup_helper(array('context' => 'entry_content','echo'=>false)).'>';
